...
volatile int Peak = 100;                         // used to find peak in pulse wave, seeded
volatile int Trough = 100;                       // used to find trough in pulse wave, seeded
volatile int Threshold = 110;                    // used to check if meeting a threshold, seeded
...
ISR(TIMER2_COMPA_vect){                         // triggered when Timer2 counts to 124
  ...
  if ((N > (IBI/5)*3) && (Signal > Threshold)){  // avoid high frequency noise
    ...
    Threshold = Peak-Trough / 2 + Trough;                   
  }
